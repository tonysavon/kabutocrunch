...
static inline unsigned short kc_limit_match_len(unsigned int len) {
   int max_len = kc_get_max_match_len();
   if (max_len > 0 && len > (unsigned int)max_len) {
...
static int salvador_find_matches_at(salvador_compressor *pCompressor, const int nOffset, salvador_match *pMatches, unsigned short *pMatchDepth, const int nMaxMatches) {
   ...
   if (matchptr < pMaxMatch) {
      ...
      if (nMatchOffset <= nMaxOffset) {
         ...
         matchptr->length = kc_limit_match_len(nMatchLen);
...
void salvador_find_all_matches(salvador_compressor *pCompressor, const int nMatchesPerOffset, const int nStartOffset, const int nEndOffset) {
   ...
   for (i = nStartOffset; i < nEndOffset; i++) {
      ...
      if (pCompressor->collect_diagnostics) {
         pCompressor->diagnostics.match_candidates_retained += (unsigned int)nMatches;
         if ((unsigned int)nMatches > pCompressor->diagnostics.max_matches_reached)
            pCompressor->diagnostics.max_matches_reached = (unsigned int)nMatches;
