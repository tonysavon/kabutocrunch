...
#include <limits.h>
#include "kc_lib.h"
#include "kc_matchfinder.h"
#include "kc_shrink.h"
#include "kc_format.h"

#define MIN_ENCODED_MATCH_SIZE   2
#define TOKEN_PARSE_COST         g_kc_token_cost
#define OFFSET_COST(__offset)    kc_offset_bit_cost(pCompressor, (__offset))

static int g_kc_max_match_len = 0;
static int g_kc_token_cost = 1;

static void *kc_alloc_array(size_t count, size_t element_size,
      size_t *allocated_bytes) {
   void *result;
   if (count && element_size > ((size_t)-1) / count) return NULL;
   result = malloc(count * element_size);
   if (result && allocated_bytes) {
      if (*allocated_bytes > ((size_t)-1) - (count * element_size)) {
         free(result);
         return NULL;
      }
      *allocated_bytes += count * element_size;
   }
   return result;
}

int kc_get_max_match_len(void) {
   return g_kc_max_match_len;
}

void kc_set_max_match_len(int nMaxMatchLen) {
   g_kc_max_match_len = nMaxMatchLen;
}

int kc_get_token_cost(void) {
   return g_kc_token_cost;
}

void kc_set_token_cost(int token_cost) {
   g_kc_token_cost = token_cost > 0 ? token_cost : 1;
}

/* Account for the selected stream's offset coding. */
static int kc_offset_bit_cost(const salvador_compressor *pCompressor,
      const int offset) {
   if (pCompressor->flags & FLG_IS_DALI) {
      return 7 + (int)kc_dali_length_bit_cost(
         (unsigned int)(((offset - 1) >> 7) + 1));
   }
   return offset <= 128 ? 8 : 16;
}

static int kc_write_bit(unsigned char* pOutData, int nOutOffset, const int nMaxOutDataSize, const int nValue, int* nCurBitsOffset, int* nCurBitShift) {
   if (nOutOffset >= 0) {
      if ((*nCurBitShift) < 0) {
...
typedef struct kc_shrink_bit_writer {
   unsigned char *data;
   int out_offset;
   int max_size;
   int *bits_offset;
   int *bit_shift;
} kc_shrink_bit_writer;

static int kc_shrink_put_bit(void *context, unsigned int bit) {
   kc_shrink_bit_writer *writer = (kc_shrink_bit_writer *)context;
   writer->out_offset = kc_write_bit(writer->data, writer->out_offset,
      writer->max_size, bit != 0, writer->bits_offset, writer->bit_shift);
   return writer->out_offset >= 0;
}

static int kc_write_length_value(unsigned char *pOutData, int nOutOffset,
      const int nMaxOutDataSize, const int nValue,
      const int nSkip, int *nCurBitsOffset, int *nCurBitShift) {
   kc_shrink_bit_writer writer;

   if (nValue <= 0)
      return -1;
   writer.data = pOutData;
   writer.out_offset = nOutOffset;
   writer.max_size = nMaxOutDataSize;
   writer.bits_offset = nCurBitsOffset;
   writer.bit_shift = nCurBitShift;
   if (!kc_write_dali_length((unsigned int)nValue, nSkip,
         kc_shrink_put_bit, &writer))
      return -1;
   return writer.out_offset;
}

static inline unsigned short kc_limit_match_len_local(int length) {
   int max_len = kc_get_max_match_len();
 ...
 */
static int salvador_write_length_value(unsigned char* pOutData, int nOutOffset,
      const int nMaxOutDataSize, const int nValue,
      const int nSkip, const int nBackward, int* nCurBitsOffset,
      int* nCurBitShift) {
   (void)nBackward;
   return kc_write_length_value(pOutData, nOutOffset, nMaxOutDataSize,
      nValue, nSkip, nCurBitsOffset, nCurBitShift);
}
 ...
 */
static inline int salvador_get_literals_varlen_size(
      const salvador_compressor *pCompressor, const int nLength) {
   (void)pCompressor;
   return nLength > 0 ? TOKEN_PARSE_COST + (int)kc_dali_length_bit_cost(
 ...
 * @return number of extra bits required
 */
static inline int salvador_get_match_varlen_size_norep(
      const salvador_compressor *pCompressor, const int nLength) {
   (void)pCompressor;
   return (int)kc_dali_length_bit_cost((unsigned int)(nLength - 1));
}
 ...
 * @return number of extra bits required
 */
static inline int salvador_get_match_varlen_size_rep(
      const salvador_compressor *pCompressor, const int nLength) {
   (void)pCompressor;
   return (int)kc_dali_length_bit_cost((unsigned int)nLength);
}
...
static void salvador_insert_forward_match(salvador_compressor *pCompressor, const unsigned char *pInWindow, const int i, const int nMatchOffset, const int nStartOffset, const int nEndOffset, const int nDepth) {
   ...
   const unsigned int nMatchesPerPosition = pCompressor->search.max_matches_per_position;
   ...
   for (j = 0; j < (int)pCompressor->search.forward_arrivals && arrival[j].from_slot; j++) {
      if (arrival[j].num_literals) {
         ...
         if (nMatchOffset != nRepOffset) {
            ...
            if (nRepPos >= nStartOffset &&
               ...
               salvador_match* fwd_match = pCompressor->match + ((nRepPos - nStartOffset) * nMatchesPerPosition);

               if (fwd_match[nMatchesPerPosition - 1].length == 0) {
                     ...
                     if (pInWindowStart[0] == pInWindowStart[-nMatchOffset]) {
                        if (nRepOffset) {
                           ...
                           while (pInWindowAtRepOffset < pInWindowMax && pInWindowAtRepOffset[0] == pInWindowAtRepOffset[-nMatchOffset])
                             ...
                             const unsigned short nCurRepLen = kc_limit_match_len_local((int)(pInWindowAtRepOffset - pInWindowStart));
                            unsigned short* fwd_depth = pCompressor->match_depth + ((nRepPos - nStartOffset) * nMatchesPerPosition);

                            if (!fwd_match[r].length) {
                                 fwd_match[r].length = nCurRepLen;
                               fwd_match[r].offset = nMatchOffset;
                               fwd_depth[r] = 0;
                               if (pCompressor->collect_diagnostics) {
                                  pCompressor->diagnostics.forward_repeat_candidates++;
                                  if ((r + 1U) > pCompressor->diagnostics.max_matches_reached)
                                     pCompressor->diagnostics.max_matches_reached = r + 1U;
                                  if ((r + 1U) == nMatchesPerPosition)
                                     pCompressor->diagnostics.match_positions_at_limit++;
                               }

                               if (nDepth < (int)pCompressor->search.forward_repeat_depth)
...
static void salvador_optimize_forward(salvador_compressor *pCompressor, const unsigned char *pInWindow, const int nStartOffset, const int nEndOffset, const int nInsertForwardReps, const int *nCurRepMatchOffset, const int nArrivalsPerPosition, const int nBlockFlags) {
   ...
   for (i = nStartOffset, cur_arrival = &arrival[nStartOffset * nMaxArrivalsPerPosition]; i != nEndOffset; i++, cur_arrival += nMaxArrivalsPerPosition) {
      ...
      for (j = 0; j < nArrivalsPerPosition && cur_arrival[j].from_slot; j++) {
         if (pCompressor->collect_diagnostics)
            pCompressor->diagnostics.arrival_candidates_considered++;
         const int nNumLiterals = cur_arrival[j].num_literals + 1;
         const int nPreviousLiteralCost = salvador_get_literals_varlen_size(
            pCompressor, nNumLiterals - 1);
         const int nCodingChoiceCost = cur_arrival[j].cost + 8 /* literal */ +
            salvador_get_literals_varlen_size(pCompressor, nNumLiterals) -
            nPreviousLiteralCost;
         const int nScore = cur_arrival[j].score + 1;
         const unsigned int nRepOffset = cur_arrival[j].rep_offset;
               ...
               if (!exists) {
                  ...
                  if (!exists) {
                     ...
                     pDestArrival->match_len = 0;
                      ...
                      if (pCompressor->collect_diagnostics)
                         pCompressor->diagnostics.arrivals_retained++;
                   }
                }
             }
            if (exists && pCompressor->collect_diagnostics)
               pCompressor->diagnostics.arrivals_discarded_duplicate++;
         }
         else if (pCompressor->collect_diagnostics) {
            pCompressor->diagnostics.arrivals_discarded_limit++;
         }
      }
      ...
      if (pCompressor->collect_diagnostics &&
          (unsigned int)nNumArrivalsForThisPos > pCompressor->diagnostics.max_arrivals_reached)
         pCompressor->diagnostics.max_arrivals_reached = (unsigned int)nNumArrivalsForThisPos;
      int nOverallMinRepLen = 0, nOverallMaxRepLen = 0;

      int *nRepMatchArrivalIdx = pCompressor->rep_match_arrival_idx;
      ...
      const unsigned int nMatchesPerPosition = pCompressor->search.max_matches_per_position;
      const salvador_match* match = pCompressor->match + ((i - nStartOffset) * nMatchesPerPosition);
      const unsigned short* match_depth = pCompressor->match_depth + ((i - nStartOffset) * nMatchesPerPosition);

      for (m = 0; m < (int)nMatchesPerPosition && match[m].length; m++) {
         ...
         for (d = 0; d <= nOrigMatchDepth; d += (nOrigMatchDepth ? nOrigMatchDepth : 1)) {
            ...
            if (nNonRepMatchArrivalIdx >= 0) {
               ...
               /* Equal-size parses prefer distance-1 matches because the
                * 6502 decoder turns their overlap into a store-only loop. */
               const int nNoRepmatchScore = cur_arrival[nNonRepMatchArrivalIdx].score +
                  (nMatchOffset == 1 ? 1 : 3);

               /* Insert non-repmatch candidate */

               if (nMatchLen < (int)pCompressor->search.exhaustive_length_threshold) {
                  nStartingMatchLen = 2;
               }
               else {
               ...
               if (pCompressor->collect_diagnostics) {
                  pCompressor->diagnostics.endpoints_evaluated +=
                     (unsigned int)(nMatchLen - nStartingMatchLen + 1);
                  if (nStartingMatchLen > 2)
                     pCompressor->diagnostics.endpoints_pruned +=
                        (unsigned int)(nStartingMatchLen - 2);
               }

               for (k = nStartingMatchLen; k <= nMatchLen; k++) {
                  salvador_arrival* pDestSlots = &cur_arrival[k * nMaxArrivalsPerPosition];
                  const int nMatchLenCost = TOKEN_PARSE_COST +
                     salvador_get_match_varlen_size_norep(pCompressor, k);
                  ...
                  if (nCodingChoiceCost < pDestSlots[nArrivalsPerPosition - 2].cost ||
                     (nCodingChoiceCost == pDestSlots[nArrivalsPerPosition - 2].cost &&
                        ...
                        (nCodingChoiceCost != pDestSlots[nArrivalsPerPosition - 1].cost || (unsigned int)nMatchOffset != pDestSlots[nArrivalsPerPosition - 1].rep_offset))) {
                     int exists = 0, n;

                     for (n = 0;
                        pDestSlots[n].cost < nCodingChoiceCost;
                        n++) {
                        if (pDestSlots[n].rep_offset == (unsigned int)nMatchOffset) {
                           exists = 1;
                           break;
                        }
                     }
                     ...
                     if (!exists) {
                        ...
                        if (!exists) {
                           ...
                           for (z = n; z < nArrivalsPerPosition - 1 && pDestSlots[z].cost == nCodingChoiceCost; z++) {
                              if (pDestSlots[z].rep_offset == (unsigned int)nMatchOffset) {
                                 exists = 1;
                                 break;
                              }
                           }
            ...
            if (nMatchLen < (int)pCompressor->search.exhaustive_length_threshold || nMatchLen <= nOverallMinRepLen) {
            ...
            for (k = nStartingMatchLen; k <= (nOverallMaxRepLen < nMatchLen ? nOverallMaxRepLen : nMatchLen); k++) {
               if (pCompressor->collect_diagnostics)
                  pCompressor->diagnostics.endpoints_evaluated++;
               const int nMatchLenCost = TOKEN_PARSE_COST +
                  salvador_get_match_varlen_size_rep(pCompressor, k);
               salvador_arrival* pDestSlots = &cur_arrival[k * nMaxArrivalsPerPosition];
               ...
               for (nCurRepMatchArrival = 0; (j = nRepMatchArrivalIdx[nCurRepMatchArrival]) >= 0; nCurRepMatchArrival += 2) {
                  if (nRepMatchArrivalIdx[nCurRepMatchArrival + 1] >= k) {
                     ...
                     const unsigned int nRepOffset = cur_arrival[j].rep_offset;
                     /* A repeated distance-1 match needs neither an offset
                      * decode nor a source load after its first byte. */
                     const int nScore = cur_arrival[j].score +
                        (nRepOffset == 1 ? 0 : 2);
                     if (pCompressor->collect_diagnostics)
                        pCompressor->diagnostics.arrival_candidates_considered++;
               ...
               if (k <= (int)pCompressor->search.exhaustive_length_threshold)
                  nOverallMinRepLen = k;
         ...
         if (pCompressor->search.enable_long_match_early_break &&
             nOrigMatchLen >= (int)pCompressor->search.long_match_break_threshold &&
             ((m + 1) >= (int)nMatchesPerPosition ||
              match[m + 1].length < pCompressor->search.remaining_match_threshold)) {
            if (pCompressor->collect_diagnostics)
               pCompressor->diagnostics.long_match_early_breaks++;
            break;
         }
      }
   }
   ...
   if (!nInsertForwardReps) {
      ...
      while (end_arrival->from_slot > 0 && end_arrival->from_pos < (const unsigned int)nEndOffset) {
         pBestMatch[end_arrival->from_pos].length = kc_limit_match_len_local(end_arrival->match_len);
   ...
   for (i = nStartOffset + (nBlockFlags & 1); i < nEndOffset; ) {
      ...
      if (pMatch->length == 0 &&
         ...
         if (pBestMatch[i + 1].offset == nRepMatchOffset) {
            nCurLenSize = salvador_get_literals_varlen_size(pCompressor, nNumLiterals + 1) + 8 + salvador_get_match_varlen_size_rep(pCompressor, pBestMatch[i + 1].length);
         }
         else {
            nCurLenSize = salvador_get_literals_varlen_size(pCompressor, nNumLiterals + 1) + 8 +
               OFFSET_COST(pBestMatch[i + 1].offset) +
               salvador_get_match_varlen_size_norep(pCompressor, pBestMatch[i + 1].length);
         }

         if (nNumLiterals != 0 && pBestMatch[i + 1].offset == nRepMatchOffset && nRepMatchOffset) {
            nReducedLenSize = salvador_get_literals_varlen_size(pCompressor, nNumLiterals) + salvador_get_match_varlen_size_rep(pCompressor, pBestMatch[i + 1].length + 1);
         }
         else {
            nReducedLenSize = salvador_get_literals_varlen_size(pCompressor, nNumLiterals) +
               OFFSET_COST(pBestMatch[i + 1].offset) +
               salvador_get_match_varlen_size_norep(pCompressor, pBestMatch[i + 1].length + 1);
         }

         if (nReducedLenSize <= nCurLenSize) {
            /* Merge */
            pBestMatch[i].length = kc_limit_match_len_local(pBestMatch[i + 1].length + 1);
      ...
      if (pMatch->length >= MIN_ENCODED_MATCH_SIZE) {
         if ((i + pMatch->length) < nEndOffset /* Don't consider the last match in the block, we can only reduce a match inbetween other tokens */) {
            ...
            if (nNextIndex < nEndOffset) {
                  ...
                  if (nNumLiterals != 0 && nRepMatchOffset && pMatch->offset != nRepMatchOffset && (pBestMatch[nNextIndex].offset != pMatch->offset ||
                     ...
                     if (i >= nRepMatchOffset &&
                        ...
                        if (nMaxLen >= 1) {
                           ...
                           nCurCommandSize = OFFSET_COST(pMatch->offset);

                           /* Match length */
                           nCurCommandSize += salvador_get_match_varlen_size_norep(pCompressor, pMatch->length);

                           /* Literals after command */
                           nCurCommandSize += salvador_get_literals_varlen_size(pCompressor, nNextLiterals);

                           /* -- Reduced: Rep match. Skip 'rep-match follows' bit. -- */

                           /* Match length */
                           nReducedCommandSize = salvador_get_match_varlen_size_rep(pCompressor, nMaxLen);

                           /* Literals after command */
                           nReducedCommandSize += (pMatch->length - nMaxLen) << 3;
                           nReducedCommandSize += salvador_get_literals_varlen_size(pCompressor, nNextLiterals + (pMatch->length - nMaxLen));
                           ...
                           if (nReducedCommandSize < nCurCommandSize) {
                              ...
                              pMatch->length = kc_limit_match_len_local(nMaxLen);
                              nDidReduce = 1;
                           }
                        }
                     }
                  ...
                  if (pBestMatch[nNextIndex].offset && pMatch->offset != pBestMatch[nNextIndex].offset && nRepMatchOffset != pBestMatch[nNextIndex].offset && nNextLiterals) {
                     ...
                     if (i >= pBestMatch[nNextIndex].offset && (i - pBestMatch[nNextIndex].offset + pMatch->length) <= nEndOffset && pMatch->offset != nRepMatchOffset) {
                        ...
                        if (nMaxLen >= pMatch->length) {
                           ...
                           nPartialSizeBefore = salvador_get_match_varlen_size_norep(pCompressor, pMatch->length);
                           nPartialSizeBefore += OFFSET_COST(pMatch->offset);
                           nPartialSizeBefore += salvador_get_literals_varlen_size(pCompressor, nNextLiterals);

                           nPartialSizeAfter = salvador_get_match_varlen_size_rep(pCompressor, nMaxLen);
                           nPartialSizeAfter += salvador_get_literals_varlen_size(pCompressor, nNextLiterals + (pMatch->length - nMaxLen)) + ((pMatch->length - nMaxLen) << 3);
               ...
               if (pMatch->length < 9 /* Don't waste time considering large matches, they will always win over literals */) {
                  ...
                  int nCurCommandSize = salvador_get_literals_varlen_size(pCompressor, nNumLiterals);
                  ...
                  if (pMatch->offset == nRepMatchOffset && nNumLiterals != 0 && nRepMatchOffset) {
                     ...
                     /* Match length */
                     nCurCommandSize += salvador_get_match_varlen_size_rep(pCompressor, pMatch->length);
                  }
                  else {
                  ...
                  int nNextCommandSize = salvador_get_literals_varlen_size(pCompressor, nNextLiterals);
                  ...
                  if (pMatch->offset && pBestMatch[nNextIndex].offset == pMatch->offset && nNextLiterals != 0) {
                     /* Match length */
                     nNextCommandSize += salvador_get_match_varlen_size_rep(pCompressor, pBestMatch[nNextIndex].length);
                  }
                  else {
                     nNextCommandSize += OFFSET_COST(pBestMatch[nNextIndex].offset);

                     /* Match length */
                     nNextCommandSize += salvador_get_match_varlen_size_norep(pCompressor, pBestMatch[nNextIndex].length);
                  }
                  ...
                  nReducedCommandSize += salvador_get_literals_varlen_size(pCompressor, nNumLiterals + pMatch->length + nNextLiterals);
                  ...
                  if (pBestMatch[nNextIndex].offset == nRepMatchOffset && (nNumLiterals + pMatch->length + nNextLiterals) != 0 && nRepMatchOffset) {
                     ...
                     /* Match length */
                     nReducedCommandSize += salvador_get_match_varlen_size_rep(pCompressor, pBestMatch[nNextIndex].length);
                  }
                  else {
                     /* Match with offset - don't include 'match with offset follows' bit  */

                     nReducedCommandSize += OFFSET_COST(pBestMatch[nNextIndex].offset);

                     /* Match length */
                     nReducedCommandSize += salvador_get_match_varlen_size_norep(pCompressor, pBestMatch[nNextIndex].length);
                  }
         ...
         if ((i + pMatch->length) < nEndOffset && pMatch->offset && pMatch->length >= MIN_ENCODED_MATCH_SIZE &&
            ...
            if (pMatch->offset == nRepMatchOffset && nNumLiterals != 0) {
               ...
               /* Match length */
               nCurPartialSize = salvador_get_match_varlen_size_rep(pCompressor, pMatch->length);
            }
            else {
               /* Match with offset. Don't include 'match with offset follows' bit. */

               nCurPartialSize = OFFSET_COST(pMatch->offset);

               /* Match length */
               nCurPartialSize += salvador_get_match_varlen_size_norep(pCompressor, pMatch->length);
            }

            /* Match with offset */
            nCurPartialSize += 1; /* match with offset */

            nCurPartialSize += OFFSET_COST(pBestMatch[i + pMatch->length].offset);

            /* Match length */
            nCurPartialSize += salvador_get_match_varlen_size_norep(pCompressor, pBestMatch[i + pMatch->length].length);
            ...
            if (nNextIndex < nEndOffset) {
               if (pBestMatch[i + pMatch->length].offset && pBestMatch[nNextIndex].offset == pBestMatch[i + pMatch->length].offset && nNextLiterals != 0) {
                  ...
                  /* Match length */
                  nCurPartialSize += salvador_get_match_varlen_size_rep(pCompressor, pBestMatch[nNextIndex].length);
               }
               else {
                  /* Match with offset. Don't include 'match with offset follows' bit. */

                  nCurPartialSize += OFFSET_COST(pBestMatch[nNextIndex].offset);

                  /* Match length */
                  nCurPartialSize += salvador_get_match_varlen_size_norep(pCompressor, pBestMatch[nNextIndex].length);
               }
            }
            ...
            if (pMatch->offset == nRepMatchOffset && nNumLiterals != 0 && nRepMatchOffset) {
               ...
               /* Match length */
               nReducedPartialSize = salvador_get_match_varlen_size_rep(pCompressor, pMatch->length + pBestMatch[i + pMatch->length].length);
            }
            else {
               /* Match with offset. Don't include 'match with offset follows' bit. */

               nReducedPartialSize = OFFSET_COST(pMatch->offset);

               /* Match length */
               nReducedPartialSize += salvador_get_match_varlen_size_norep(pCompressor, pMatch->length + pBestMatch[i + pMatch->length].length);
            }
            ...
            if (nNextIndex < nEndOffset) {
               if (pMatch->offset && pBestMatch[nNextIndex].offset == pMatch->offset && nNextLiterals != 0) {
                  ...
                  /* Match length */
                  nReducedPartialSize += salvador_get_match_varlen_size_rep(pCompressor, pBestMatch[nNextIndex].length);
               }
               else {
                  if (pBestMatch[nNextIndex].length >= MIN_ENCODED_MATCH_SIZE) {
                     /* Match with offset. Don't include 'match with offset follows' bit. */

                     nReducedPartialSize += OFFSET_COST(pBestMatch[nNextIndex].offset);

                     /* Match length */
                     nReducedPartialSize += salvador_get_match_varlen_size_norep(pCompressor, pBestMatch[nNextIndex].length);
                  }
                  else {
                     nCannotReduce = 1;
                  }
            ...
            if (nCurPartialSize >= nReducedPartialSize && !nCannotReduce) {
               ...
               pMatch->length = kc_limit_match_len_local(pMatch->length + pBestMatch[i + nMatchLen].length);
            ...
            if ((i + MIN_ENCODED_MATCH_SIZE /* pMatch->length */) < nEndOffset) {
               ...
               if (nNextIndex < nEndOffset && nNextLiterals != 0 &&
                  ...
                  if (nNextNextIndex < nEndOffset && nNextNextLiterals != 0 &&
                     ...
                     nCurCommandSize = salvador_get_literals_varlen_size(pCompressor, nNumLiterals);
                     ...
                     nCurRepMatchSize = salvador_get_literals_varlen_size(pCompressor, nNextLiterals);
                     ...
                     nReducedCommandSize = salvador_get_literals_varlen_size(pCompressor, nNumLiterals + MIN_ENCODED_MATCH_SIZE /* pMatch->length */ + nNextLiterals + 1 /* pBestMatch[nNextIndex].length */);
         ...
         if (nNumLiterals != 0) {
            ...
            if (nNextLiterals != 0) {
               int nCurPartialSize = salvador_get_literals_varlen_size(pCompressor, nNumLiterals);
               nCurPartialSize += TOKEN_PARSE_COST + 1 /* salvador_get_match_varlen_size_rep(pMatch->length) */;
               nCurPartialSize += salvador_get_literals_varlen_size(pCompressor, nNextLiterals);

               const int nReducedPartialSize = salvador_get_literals_varlen_size(pCompressor, nNumLiterals + 1 + nNextLiterals) + 8;
...
static int salvador_write_literals(salvador_compressor* pCompressor, const unsigned char* pInWindow, const int nLiteralOffset, const int nNumLiterals, unsigned char* pOutData, const int nMaxOutDataSize, int nOutOffset, int* nCurBitsOffset, int* nCurBitShift, int* nIsFirstCommand, const int nIsBackward) {
   ...
   if (!*nIsFirstCommand) {
      nOutOffset = kc_write_bit(pOutData, nOutOffset, nMaxOutDataSize, 0 /* literals follow */, nCurBitsOffset, nCurBitShift);
      if (nOutOffset < 0) return -1;
   }

   *nIsFirstCommand = 0;

   nOutOffset = salvador_write_length_value(pOutData, nOutOffset,
      nMaxOutDataSize, nNumLiterals, 0,
      nIsBackward, nCurBitsOffset, nCurBitShift);
   if (nOutOffset < 0) return -1;

   if ((nOutOffset + nNumLiterals) > nMaxOutDataSize)
      return -1;
   pCompressor->stats.nonzero_literal_runs++;
   memcpy(pOutData + nOutOffset, pInWindow + nLiteralOffset, nNumLiterals);
...
static int salvador_write_block(salvador_compressor* pCompressor, const unsigned char* pInWindow, const int nStartOffset, const int nEndOffset, unsigned char* pOutData, const int nMaxOutDataSize, int* nCurBitsOffset, int* nCurBitShift, int* nFinalLiterals, int* nCurRepMatchOffset, const int nBlockFlags) {
   ...
   for (i = nStartOffset; i < nEndOffset; ) {
      ...
      if (pMatch->length >= 2 || (pMatch->length == 1 && pMatch->offset == nRepMatchOffset && nNumLiterals != 0)) {
         ...
         if (nNumLiterals != 0) {
            /* Literals */
            nOutOffset = salvador_write_literals(pCompressor, pInWindow, nInFirstLiteralOffset, nNumLiterals, pOutData, nMaxOutDataSize, nOutOffset, nCurBitsOffset, nCurBitShift, &nIsFirstCommand, nIsBackward);
            if (nOutOffset < 0) return -1;
        }

         if (nMatchOffset == nRepMatchOffset && nNumLiterals != 0) {
            /* Rep match */
            nOutOffset = kc_write_bit(pOutData, nOutOffset, nMaxOutDataSize, 0 /* rep match */, nCurBitsOffset, nCurBitShift);
            if (nOutOffset < 0) return -1;

            /* Write match length */
            nOutOffset = kc_write_length_value(pOutData,
               nOutOffset, nMaxOutDataSize, nMatchLen, 0,
               nCurBitsOffset, nCurBitShift);
            if (nOutOffset < 0) return -1;
         }
         else {
             /* Match with offset */
             nOutOffset = kc_write_bit(pOutData, nOutOffset, nMaxOutDataSize, 1 /* match with offset */, nCurBitsOffset, nCurBitShift);
             if (nOutOffset < 0) return -1;

             const int nOffsetHigh = (nMatchOffset - 1) >> 7;
             if (pCompressor->flags & FLG_IS_DALI) {
                nOutOffset = kc_write_length_value(pOutData, nOutOffset,
                   nMaxOutDataSize, nOffsetHigh + 1, 0,
                   nCurBitsOffset, nCurBitShift);
                if (nOutOffset < 0) return -1;
             }
             else {
                ...
                nOutOffset = kc_write_bit(pOutData, nOutOffset,
                   nMaxOutDataSize, nOffsetHigh == 0,
                   nCurBitsOffset, nCurBitShift);
                if (nOutOffset < 0) return -1;

                if (nOffsetHigh != 0) {
                   if (nOutOffset >= nMaxOutDataSize || nOffsetHigh >= 0xff)
                      return -1;
                   pOutData[nOutOffset++] = (unsigned char)nOffsetHigh;
                }
             }
             ...
             if (nMatchLen == 2) {
                ...
                nOutOffset = kc_write_length_value(pOutData,
                   nOutOffset, nMaxOutDataSize, nMatchLen - 1, 1,
                   nCurBitsOffset,
                   nCurBitShift);
                if (nOutOffset < 0) return -1;
             }
   ...
   if (nBlockFlags & 2) {
      *nFinalLiterals = 0;
      ...
      if (pCompressor->flags & FLG_IS_DALI) {
         nOutOffset = kc_write_length_value(pOutData, nOutOffset,
            nMaxOutDataSize, 256 /* EOD */, 0,
            nCurBitsOffset, nCurBitShift);
         if (nOutOffset < 0) return -1;
      }
      else {
         nOutOffset = kc_write_bit(pOutData, nOutOffset, nMaxOutDataSize,
            0 /* long-offset selector */, nCurBitsOffset, nCurBitShift);
         if (nOutOffset < 0) return -1;
         if (nOutOffset >= nMaxOutDataSize) return -1;
         pOutData[nOutOffset++] = 0x00; /* invalid long-offset high: EOD */
      }
...
static int salvador_optimize_and_write_block(salvador_compressor *pCompressor, const unsigned char *pInWindow, const int nPreviousBlockSize, const int nInDataSize, unsigned char *pOutData, const int nMaxOutDataSize, int *nCurBitsOffset, int *nCurBitShift, int *nFinalLiterals, int *nCurRepMatchOffset, const int nBlockFlags) {
   ...
   for (nPosition = nPreviousBlockSize + 1; nPosition < (nEndOffset - 1); nPosition++) {
      salvador_match *match = pCompressor->match + ((nPosition - nPreviousBlockSize) * nMatchesPerPosition);
      const int nSupplementLength = (int)pCompressor->search.supplement_match_length_limit;
      const int nMaxMatchLen = ((nPosition + nSupplementLength) < nEndOffset) ? nSupplementLength : (nEndOffset - nPosition);
      const unsigned char* pInWindowMax = pInWindow + nPosition + nMaxMatchLen;
      const unsigned char* pInWindowStart = pInWindow + nPosition;
      unsigned short *match_depth = pCompressor->match_depth + ((nPosition - nPreviousBlockSize) * nMatchesPerPosition);
      int m = 0;
      int nMatchPos;

      while (m < (int)pCompressor->search.initial_supplement_matches && match[m].length) {
         offset_cache[match[m].offset & 2047] = nPosition;
         offset_cache[(match[m].offset - match_depth[m]) & 2047] = nPosition;
         m++;
      }

      for (nMatchPos = next_offset_for_pos[nPosition - nPreviousBlockSize];
           m < (int)pCompressor->search.initial_supplement_matches && nMatchPos >= 0;
         ...
         if (nMatchOffset <= pCompressor->max_offset) {
            ...
            if (!nAlreadyExists) {
               ...
               match[m].length = kc_limit_match_len_local((int)(pInWindowAtPos - pInWindowStart));
               match[m].offset = nMatchOffset;
               match_depth[m] = 0;
               m++;
               if (pCompressor->collect_diagnostics &&
                   (unsigned int)m > pCompressor->diagnostics.max_matches_reached)
                  pCompressor->diagnostics.max_matches_reached = (unsigned int)m;
               if (pCompressor->collect_diagnostics &&
                   (unsigned int)m == nMatchesPerPosition)
                  pCompressor->diagnostics.match_positions_at_limit++;
            }
         }
         else {
   ...
   salvador_optimize_forward(pCompressor, pInWindow, nPreviousBlockSize, nEndOffset,
      1 /* nInsertForwardReps */, nCurRepMatchOffset,
      (int)pCompressor->search.forward_arrivals, nBlockFlags);
   ...
   for (nPosition = nPreviousBlockSize + 1; nPosition < (nEndOffset - 1); nPosition++) {
      ...
      if (match[0].length < pCompressor->search.supplement_trigger_length) {
         const int nSupplementLength = (int)pCompressor->search.supplement_match_length_limit;
         const int nMaxMatchLen = ((nPosition + nSupplementLength) < nEndOffset) ? nSupplementLength : (nEndOffset - nPosition);
         const unsigned char* pInWindowMax = pInWindow + nPosition + nMaxMatchLen;
         const unsigned char* pInWindowStart = pInWindow + nPosition;
         ...
         int m = 0, nInserted = 0;
         int nMatchPos;
         int nMaxForwardPos = nPosition + 2 + 1 + (int)pCompressor->search.forward_search_distance;

         if (nMaxForwardPos > (nEndOffset - 2))
            nMaxForwardPos = nEndOffset - 2;

         while (m < (int)nMatchesPerPosition && match[m].length) {
            offset_cache[match[m].offset & 2047] = nPosition;
            offset_cache[(match[m].offset - match_depth[m]) & 2047] = nPosition;
            m++;
         }
         ...
         for (nMatchPos = next_offset_for_pos[nPosition - nPreviousBlockSize];
              m < (int)nMatchesPerPosition && nMatchPos >= 0;
              nMatchPos = next_offset_for_pos[nMatchPos - nPreviousBlockSize]) {
            const int nMatchOffset = nPosition - nMatchPos;
               ...
               if (!nAlreadyExists) {
                  ...
                  if (nForwardPos >= nMatchOffset) {
                     ...
                     if (nForwardPos < nMaxForwardPos) {
                        ...
                        nInserted++;
                        if (nInserted >= (int)pCompressor->search.forward_insert_limit)
                           break;
                     }
                  }
               }
   ...
   do {
      ...
      nPasses++;
   } while (nDidReduce && nPasses < (int)pCompressor->search.reduction_passes);
 ...
 */
static int salvador_compressor_init(salvador_compressor *pCompressor,
      const int nBlockSize, const int nMaxWindowSize, const size_t nMaxOffset,
      const int nFlags, const kc_search_config *search_config) {
   int nResult;
   ...
   pCompressor->rep_match_arrival_idx = NULL;
   ...
   pCompressor->search = *search_config;
   pCompressor->max_arrivals_per_position = (int)search_config->max_arrivals_per_position;
   pCompressor->collect_diagnostics = 0;

   memset(&pCompressor->stats, 0, sizeof(pCompressor->stats));
   memset(&pCompressor->diagnostics, 0, sizeof(pCompressor->diagnostics));
   ...
   if (!nResult) {
      pCompressor->intervals = (unsigned long long *)kc_alloc_array(
         (size_t)nMaxWindowSize, sizeof(unsigned long long),
         &pCompressor->diagnostics.allocated_bytes);

      if (pCompressor->intervals) {
         pCompressor->pos_data = (unsigned long long *)kc_alloc_array(
            (size_t)nMaxWindowSize, sizeof(unsigned long long),
            &pCompressor->diagnostics.allocated_bytes);

         if (pCompressor->pos_data) {
            pCompressor->open_intervals = (unsigned long long *)kc_alloc_array(
               LCP_AND_TAG_MAX + 1U, sizeof(unsigned long long),
               &pCompressor->diagnostics.allocated_bytes);

            if (pCompressor->open_intervals) {
               pCompressor->arrival = (salvador_arrival *)kc_alloc_array(
                  (size_t)nBlockSize + 1U,
                  (size_t)search_config->max_arrivals_per_position * sizeof(salvador_arrival),
                  &pCompressor->diagnostics.allocated_bytes);

               if (pCompressor->arrival) {
                  pCompressor->best_match = (salvador_match *)kc_alloc_array(
                     (size_t)nBlockSize, sizeof(salvador_match),
                     &pCompressor->diagnostics.allocated_bytes);

                  if (pCompressor->best_match) {
                     pCompressor->match = (salvador_match *)kc_alloc_array(
                        (size_t)nBlockSize,
                        (size_t)search_config->max_matches_per_position * sizeof(salvador_match),
                        &pCompressor->diagnostics.allocated_bytes);
                     if (pCompressor->match) {
                        pCompressor->match_depth = (unsigned short *)kc_alloc_array(
                           (size_t)nBlockSize,
                           (size_t)search_config->max_matches_per_position * sizeof(unsigned short),
                           &pCompressor->diagnostics.allocated_bytes);
                        if (pCompressor->match_depth) {
                           pCompressor->first_offset_for_byte = (int*)kc_alloc_array(
                              65536, sizeof(int), &pCompressor->diagnostics.allocated_bytes);
                           if (pCompressor->first_offset_for_byte) {
                              pCompressor->next_offset_for_pos = (int*)kc_alloc_array(
                                 (size_t)nBlockSize, sizeof(int),
                                 &pCompressor->diagnostics.allocated_bytes);
                              if (pCompressor->next_offset_for_pos) {
                                 pCompressor->offset_cache = (int*)kc_alloc_array(
                                    2048, sizeof(int), &pCompressor->diagnostics.allocated_bytes);
                                 if (pCompressor->offset_cache) {
                                    pCompressor->rep_match_arrival_idx = (int*)kc_alloc_array(
                                       ((size_t)search_config->max_arrivals_per_position * 2U) + 1U,
                                       sizeof(int), &pCompressor->diagnostics.allocated_bytes);
                                    if (pCompressor->rep_match_arrival_idx) return 0;
                                 }
                              }
                           }
                        }
...
static void salvador_compressor_destroy(salvador_compressor *pCompressor) {
   ...
   if (pCompressor->rep_match_arrival_idx) {
      free(pCompressor->rep_match_arrival_idx);
      pCompressor->rep_match_arrival_idx = NULL;
   }
...
static int salvador_compressor_shrink_block(salvador_compressor *pCompressor, const unsigned char *pInWindow, const int nPreviousBlockSize, const int nInDataSize, unsigned char *pOutData, const int nMaxOutDataSize, int *nCurBitsOffset, int *nCurBitShift, int *nFinalLiterals, int *nCurRepMatchOffset, const int nBlockFlags) {
   ...
   if (salvador_build_suffix_array(pCompressor, pInWindow, nPreviousBlockSize + nInDataSize))
      ...
      salvador_find_all_matches(pCompressor,
         (int)pCompressor->search.max_matches_per_position,
         nPreviousBlockSize, nPreviousBlockSize + nInDataSize);
 ...
 */
size_t salvador_compress(const unsigned char *pInputData,
      unsigned char *pOutBuffer, const size_t nInputSize,
      const size_t nMaxOutBufferSize, const kc_search_config *search_config,
      const unsigned int nFlags,
      const size_t nMaxOffset, const size_t nDictionarySize,
      void(*progress)(long long nOriginalSize, long long nCompressedSize),
      salvador_stats *pStats, kc_search_diagnostics *pSearchDiagnostics) {
   ...
   nResult = salvador_compressor_init(&compressor, nBlockSize,
      nBlockSize * 2, nMaxOffset, nFlags, search_config);
   if (nResult != 0) {
      return -1;
   }
   compressor.collect_diagnostics = pSearchDiagnostics != NULL;
   ...
   if (pSearchDiagnostics)
      *pSearchDiagnostics = compressor.diagnostics;
