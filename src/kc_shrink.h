...
typedef struct _salvador_compressor {
   ...
   int *offset_cache;
   int *rep_match_arrival_idx;
   int flags;
   int block_size;
   int max_offset;
   int max_arrivals_per_position;
   kc_search_config search;
   kc_search_diagnostics diagnostics;
...
/* Virtual command-bit cost used only by the parser. The writer always emits
 * the selected grammar regardless of this setting. */
 ...
 * @param nFlags compression flags (FLG_IS_INVERTED, optionally FLG_IS_DALI)
 ...
 */
size_t salvador_compress(const unsigned char *pInputData, unsigned char *pOutBuffer,
   const size_t nInputSize, const size_t nMaxOutBufferSize,
   const kc_search_config *search_config,
   const unsigned int nFlags, const size_t nMaxOffset,
